...
class WeChatExternalContact(BaseWeChatAPI):
    ...
    def get_follow_user_list(self) -> dict:
        ...
        .. code-block:: python
            ...
            from findy.vendor.wechatpy.work import WeChatClient
        ...
        .. note::
            **权限说明：**
            ...
            - 需要使用 `客户联系secret`_ 或配置到 `可调用应用`_ 列表中的自建应用secret
              来初始化 :py:class:`findy.vendor.wechatpy.work.client.WeChatClient` 类。
            - 第三方应用需拥有“企业客户”权限。
    ...
    def add_msg_template(self, template: dict) -> dict:
        ...
        **注意**：调用该接口并不会直接发送消息给客户/客户群，需要相关的客服人员操作以后才会
        ...
        .. code-block:: python
            ...
            from findy.vendor.wechatpy.exceptions import WeChatClientException
            from findy.vendor.wechatpy.work import WeChatClient

    ...
    def run(self):
        ...
        for category, url in self.category_map_url.items():
            ...
            if the_list:
                ...
                df_to_db(df=df, region=Region.CHN, data_schema=self.data_schema, provider=self.provider, force_update=True)
...
class EastmoneyChinaBlockStockRecorder(TimeSeriesDataRecorder):
    ...
    def record(self, entity, start, end, size, timestamps, http_session):
        resp = request_get(http_session, self.category_stocks_url.format(entity.code, '1'))
        try:
            results = json_callback_param(resp.text)
            the_list = []
            for result in results:

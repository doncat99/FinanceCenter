...
class interface():
    @staticmethod
    def get_stock_list_data(region: Region, provider: Provider):
        # 股票列表
        Stock.record_data(region=region, provider=provider, sleeping_time=0)

    @staticmethod
    def get_etf_list_data(region: Region, provider: Provider):
        Etf.record_data(region=region, provider=provider, sleeping_time=0)

    @staticmethod
    def get_stock_trade_day(region: Region, provider: Provider, process, mode):
        # 交易日
        StockTradeDay.record_data(region=region, provider=provider, share_para=(process, 'Trade Day', mode), sleeping_time=0)

    @staticmethod
    def get_stock_main_index(region: Region, provider: Provider):
...
def fetch_data(region: Region):
    ...
    if region == Region.CHN:
        ...
        interface.get_stock_list_data(region, Provider.BaoStock)
        interface.get_stock_trade_day(region, Provider.BaoStock, 1, RunMode.Sync)
        interface.get_etf_list_data(region, Provider.JoinQuant)
        interface.get_stock_main_index(region, Provider.JoinQuant)

    elif region == Region.US:
        data_set = data_set_us
        interface.get_stock_list_data(region, Provider.Yahoo)
        interface.get_stock_trade_day(region, Provider.Yahoo, 1, RunMode.Sync)
